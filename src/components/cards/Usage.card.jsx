...
const UsageCard = () => {
    ...
    return (<div className="row g-4">
        ...
        {Object.values(usages ?? {}).map((usage, index) => {
            const progress = Math.ceil((usage.size / usage.limit) * 100).toFixed(0);
            ...
            return (<div key={index} className="col-12 small">
                <div className="d-flex small fw-bold mb-1 gap-2">
                    ...
                    <span> / </span>
                    <span>{usage.limit >= 999999 ? <i className="bi bi-infinity text-danger-emphasis"></i> : usage.limit}</span>
                </div>
